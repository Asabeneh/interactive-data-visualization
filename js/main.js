...
const createCountryUI = ({ name, capital, languages, population, flag }) => {
  ...
  return `<div class="country">
            ...
            <div class="country_text">
              <p>${formatedCapital}</p>
              <p><span>${formatLanguage}: </span>${languages.map((lang) => lang.name).join(', ')}</p>
...
const filterCountries = (arr, search) => {
  ...
  const filteredCountries = arr.filter(country => {
    ...
    const isLanguages = languages.map((lang) => lang.name)
      .join()
      .toLowerCase()
      .includes(searchTerm)
...
const countLanguages = arr => {
  ...
  arr.forEach(country => {
    ...
    for (const language of languages.map((lang) => lang.name)) {
      langs.push(language)
